...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template match="//member[contains(@name,'T:')]">
    ...
    "FullName":"<xsl:value-of select="$FullMemberName"/>",
    ...
    <xsl:if test="//member[contains(@name,concat('F:',$FullMemberName))]">
      ...
      <xsl:for-each select="//member[contains(@name,concat('F:',$FullMemberName))]">
        {
          "FieldName":"<xsl:value-of select="substring-after(@name, concat('F:',$FullMemberName,'.'))"/>",
          "FieldDescription":"<xsl:apply-templates />"
        },
      </xsl:for-each>
      ],
    </xsl:if>
    ...
    <xsl:if test="//member[contains(@name,concat('P:',$FullMemberName))]">
      ...
      <xsl:for-each select="//member[contains(@name,concat('P:',$FullMemberName))]">
        {
          "PropertyName":"<xsl:value-of select="substring-after(@name, concat('P:',$FullMemberName,'.'))"/>",
          "PropertyDescription":"<xsl:apply-templates />"
        },
      </xsl:for-each>
      ],
    </xsl:if>
